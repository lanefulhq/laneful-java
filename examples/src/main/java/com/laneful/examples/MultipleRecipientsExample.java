...
public class MultipleRecipientsExample {
    public static void main(String[] args) {
        ...
        if (recipients.length < 2) {
            System.err.println("❌ Need at least 2 recipient emails for this example");
            System.exit(1);
        }
        ...
        try {
            ...
            for (int i = 0; i < Math.min(recipients.length, 2); i++) {
                emailBuilder.to(new Address(recipients[i].trim(), "User " + (i + 1)));
            }
            
            // Add CC if we have more recipients
            if (recipients.length > 2) {
                emailBuilder.cc(new Address(recipients[2].trim(), "CC Recipient"));
            }
            
            // Add BCC if we have even more recipients
            if (recipients.length > 3) {
                emailBuilder.bcc(new Address(recipients[3].trim(), "BCC Recipient"));
            }
            
            // Add reply-to
            emailBuilder.replyTo(new Address(fromEmail, "Reply To"));
